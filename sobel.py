...
hx = np.array([[-1/4,0,1/4],[-1/2,0,1/2],[-1/4,0,1/4]])
hy = np.array([[-1/4,-1/2,-1/4],[0,0,0],[1/4,1/2,1/4]])
img = (data.coins())
img_outx = img.copy() 
img_outy = img.copy()  
line , row = img.shape
...
cv2.imshow('image_x',img_outx)
cv2.imshow('image_y',img_outx)
cv2.imwrite('sobelx.png',img_outx)
cv2.imwrite('sobely.png',img_outx)

"""
img_outx = Image.fromarray(img_outx)
img_outx = img_outx.convert('RGB')
scipy.misc.imsave('sobelx.png', img_outx)
img_outy = Image.fromarray(img_outy)
img_outy = img_outy.convert('RGB')
scipy.misc.imsave('sobely.png', img_outy)
img_outx.show()
#img_outy.show()
"""